  ...
  return (
    <div className="min-h-screen bg-background pb-16">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {user && (user.educationLevel || user.fieldOfStudy) && user.educationVisibility !== "hide_all" && (
          <Card className="border shadow-sm">
            ...
            <CardContent className="space-y-2 text-sm">
              {user.educationLevel && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">教育水平</span>
                  <span data-testid="text-education-level">{getEducationDisplay(user.educationLevel)}</span>
                </div>
              )}
              {user.fieldOfStudy && user.educationVisibility === "show_level_and_field" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">专业领域</span>
                  <span data-testid="text-field-of-study">{user.fieldOfStudy}</span>
                </div>
              )}
              {user.studyLocale && user.educationVisibility === "show_level_and_field" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">学习地点</span>
                  <span data-testid="text-study-locale">{getStudyLocaleDisplay(user.studyLocale)}</span>
                </div>
              )}
              {user.overseasRegions && user.overseasRegions.length > 0 && user.educationVisibility === "show_level_and_field" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">海外地区</span>
                  <span data-testid="text-overseas-regions">{user.overseasRegions.join(", ")}</span>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        {/* Work Information */}
        {user && user.industry && user.workVisibility === "show_industry_only" && (
          <Card className="border shadow-sm">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {user.roleTitleShort && (
                <div className="flex justify-between">
                  ...
                  <span data-testid="text-role-title">{user.roleTitleShort}</span>
                </div>
              )}
              {user.seniority && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">资历</span>
                  <span data-testid="text-seniority">{getSeniorityDisplay(user.seniority)}</span>
                </div>
              )}
            </CardContent>
          </Card>
