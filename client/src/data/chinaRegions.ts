/**
 * 中国省市数据（含港澳台）
 * 用于家乡选择的省市联动
 */

export interface City {
  name: string;
  code: string;
}

export interface Province {
  name: string;
  code: string;
  cities: City[];
}

export const chinaRegions: Province[] = [
  // 直辖市
  {
    name: "北京市",
    code: "11",
    cities: [
      { name: "北京市", code: "1100" }
    ]
  },
  {
    name: "天津市",
    code: "12",
    cities: [
      { name: "天津市", code: "1200" }
    ]
  },
  {
    name: "上海市",
    code: "31",
    cities: [
      { name: "上海市", code: "3100" }
    ]
  },
  {
    name: "重庆市",
    code: "50",
    cities: [
      { name: "重庆市", code: "5000" }
    ]
  },
  
  // 省份
  {
    name: "广东省",
    code: "44",
    cities: [
      { name: "广州市", code: "4401" },
      { name: "深圳市", code: "4403" },
      { name: "珠海市", code: "4404" },
      { name: "汕头市", code: "4405" },
      { name: "佛山市", code: "4406" },
      { name: "东莞市", code: "4419" },
      { name: "中山市", code: "4420" },
      { name: "惠州市", code: "4413" },
      { name: "江门市", code: "4407" },
      { name: "湛江市", code: "4408" },
      { name: "茂名市", code: "4409" },
      { name: "肇庆市", code: "4412" },
      { name: "梅州市", code: "4414" },
      { name: "汕尾市", code: "4415" },
      { name: "河源市", code: "4416" },
      { name: "阳江市", code: "4417" },
      { name: "清远市", code: "4418" },
      { name: "潮州市", code: "4451" },
      { name: "揭阳市", code: "4452" },
      { name: "云浮市", code: "4453" },
    ]
  },
  {
    name: "江苏省",
    code: "32",
    cities: [
      { name: "南京市", code: "3201" },
      { name: "苏州市", code: "3205" },
      { name: "无锡市", code: "3202" },
      { name: "常州市", code: "3204" },
      { name: "南通市", code: "3206" },
      { name: "扬州市", code: "3210" },
      { name: "镇江市", code: "3211" },
      { name: "盐城市", code: "3209" },
      { name: "泰州市", code: "3212" },
      { name: "徐州市", code: "3203" },
      { name: "淮安市", code: "3208" },
      { name: "连云港市", code: "3207" },
      { name: "宿迁市", code: "3213" },
    ]
  },
  {
    name: "浙江省",
    code: "33",
    cities: [
      { name: "杭州市", code: "3301" },
      { name: "宁波市", code: "3302" },
      { name: "温州市", code: "3303" },
      { name: "嘉兴市", code: "3304" },
      { name: "湖州市", code: "3305" },
      { name: "绍兴市", code: "3306" },
      { name: "金华市", code: "3307" },
      { name: "衢州市", code: "3308" },
      { name: "舟山市", code: "3309" },
      { name: "台州市", code: "3310" },
      { name: "丽水市", code: "3311" },
    ]
  },
  {
    name: "山东省",
    code: "37",
    cities: [
      { name: "济南市", code: "3701" },
      { name: "青岛市", code: "3702" },
      { name: "烟台市", code: "3706" },
      { name: "威海市", code: "3710" },
      { name: "潍坊市", code: "3707" },
      { name: "淄博市", code: "3703" },
      { name: "临沂市", code: "3713" },
      { name: "济宁市", code: "3708" },
      { name: "泰安市", code: "3709" },
      { name: "日照市", code: "3711" },
      { name: "德州市", code: "3714" },
      { name: "聊城市", code: "3715" },
      { name: "滨州市", code: "3716" },
      { name: "菏泽市", code: "3717" },
      { name: "枣庄市", code: "3704" },
      { name: "东营市", code: "3705" },
    ]
  },
  {
    name: "河南省",
    code: "41",
    cities: [
      { name: "郑州市", code: "4101" },
      { name: "洛阳市", code: "4103" },
      { name: "开封市", code: "4102" },
      { name: "新乡市", code: "4107" },
      { name: "安阳市", code: "4105" },
      { name: "焦作市", code: "4108" },
      { name: "许昌市", code: "4110" },
      { name: "南阳市", code: "4113" },
      { name: "信阳市", code: "4115" },
      { name: "周口市", code: "4116" },
      { name: "驻马店市", code: "4117" },
      { name: "商丘市", code: "4114" },
      { name: "平顶山市", code: "4104" },
      { name: "濮阳市", code: "4109" },
      { name: "鹤壁市", code: "4106" },
      { name: "漯河市", code: "4111" },
      { name: "三门峡市", code: "4112" },
    ]
  },
  {
    name: "四川省",
    code: "51",
    cities: [
      { name: "成都市", code: "5101" },
      { name: "绵阳市", code: "5107" },
      { name: "德阳市", code: "5106" },
      { name: "宜宾市", code: "5115" },
      { name: "泸州市", code: "5105" },
      { name: "南充市", code: "5113" },
      { name: "乐山市", code: "5111" },
      { name: "达州市", code: "5117" },
      { name: "内江市", code: "5110" },
      { name: "遂宁市", code: "5109" },
      { name: "攀枝花市", code: "5104" },
      { name: "自贡市", code: "5103" },
      { name: "眉山市", code: "5114" },
      { name: "广安市", code: "5116" },
      { name: "资阳市", code: "5120" },
      { name: "广元市", code: "5108" },
      { name: "雅安市", code: "5118" },
      { name: "巴中市", code: "5119" },
    ]
  },
  {
    name: "湖北省",
    code: "42",
    cities: [
      { name: "武汉市", code: "4201" },
      { name: "宜昌市", code: "4205" },
      { name: "襄阳市", code: "4206" },
      { name: "荆州市", code: "4210" },
      { name: "黄石市", code: "4202" },
      { name: "十堰市", code: "4203" },
      { name: "荆门市", code: "4208" },
      { name: "鄂州市", code: "4207" },
      { name: "孝感市", code: "4209" },
      { name: "黄冈市", code: "4211" },
      { name: "咸宁市", code: "4212" },
      { name: "随州市", code: "4213" },
      { name: "恩施州", code: "4228" },
    ]
  },
  {
    name: "湖南省",
    code: "43",
    cities: [
      { name: "长沙市", code: "4301" },
      { name: "株洲市", code: "4302" },
      { name: "湘潭市", code: "4303" },
      { name: "衡阳市", code: "4304" },
      { name: "邵阳市", code: "4305" },
      { name: "岳阳市", code: "4306" },
      { name: "常德市", code: "4307" },
      { name: "张家界市", code: "4308" },
      { name: "益阳市", code: "4309" },
      { name: "郴州市", code: "4310" },
      { name: "永州市", code: "4311" },
      { name: "怀化市", code: "4312" },
      { name: "娄底市", code: "4313" },
      { name: "湘西州", code: "4331" },
    ]
  },
  {
    name: "福建省",
    code: "35",
    cities: [
      { name: "福州市", code: "3501" },
      { name: "厦门市", code: "3502" },
      { name: "泉州市", code: "3505" },
      { name: "漳州市", code: "3506" },
      { name: "莆田市", code: "3503" },
      { name: "龙岩市", code: "3508" },
      { name: "三明市", code: "3504" },
      { name: "南平市", code: "3507" },
      { name: "宁德市", code: "3509" },
    ]
  },
  {
    name: "安徽省",
    code: "34",
    cities: [
      { name: "合肥市", code: "3401" },
      { name: "芜湖市", code: "3402" },
      { name: "蚌埠市", code: "3403" },
      { name: "淮南市", code: "3404" },
      { name: "马鞍山市", code: "3405" },
      { name: "淮北市", code: "3406" },
      { name: "铜陵市", code: "3407" },
      { name: "安庆市", code: "3408" },
      { name: "黄山市", code: "3410" },
      { name: "滁州市", code: "3411" },
      { name: "阜阳市", code: "3412" },
      { name: "宿州市", code: "3413" },
      { name: "六安市", code: "3415" },
      { name: "亳州市", code: "3416" },
      { name: "池州市", code: "3417" },
      { name: "宣城市", code: "3418" },
    ]
  },
  {
    name: "江西省",
    code: "36",
    cities: [
      { name: "南昌市", code: "3601" },
      { name: "景德镇市", code: "3602" },
      { name: "萍乡市", code: "3603" },
      { name: "九江市", code: "3604" },
      { name: "新余市", code: "3605" },
      { name: "鹰潭市", code: "3606" },
      { name: "赣州市", code: "3607" },
      { name: "吉安市", code: "3608" },
      { name: "宜春市", code: "3609" },
      { name: "抚州市", code: "3610" },
      { name: "上饶市", code: "3611" },
    ]
  },
  {
    name: "河北省",
    code: "13",
    cities: [
      { name: "石家庄市", code: "1301" },
      { name: "唐山市", code: "1302" },
      { name: "秦皇岛市", code: "1303" },
      { name: "邯郸市", code: "1304" },
      { name: "邢台市", code: "1305" },
      { name: "保定市", code: "1306" },
      { name: "张家口市", code: "1307" },
      { name: "承德市", code: "1308" },
      { name: "沧州市", code: "1309" },
      { name: "廊坊市", code: "1310" },
      { name: "衡水市", code: "1311" },
    ]
  },
  {
    name: "山西省",
    code: "14",
    cities: [
      { name: "太原市", code: "1401" },
      { name: "大同市", code: "1402" },
      { name: "阳泉市", code: "1403" },
      { name: "长治市", code: "1404" },
      { name: "晋城市", code: "1405" },
      { name: "朔州市", code: "1406" },
      { name: "晋中市", code: "1407" },
      { name: "运城市", code: "1408" },
      { name: "忻州市", code: "1409" },
      { name: "临汾市", code: "1410" },
      { name: "吕梁市", code: "1411" },
    ]
  },
  {
    name: "辽宁省",
    code: "21",
    cities: [
      { name: "沈阳市", code: "2101" },
      { name: "大连市", code: "2102" },
      { name: "鞍山市", code: "2103" },
      { name: "抚顺市", code: "2104" },
      { name: "本溪市", code: "2105" },
      { name: "丹东市", code: "2106" },
      { name: "锦州市", code: "2107" },
      { name: "营口市", code: "2108" },
      { name: "阜新市", code: "2109" },
      { name: "辽阳市", code: "2110" },
      { name: "盘锦市", code: "2111" },
      { name: "铁岭市", code: "2112" },
      { name: "朝阳市", code: "2113" },
      { name: "葫芦岛市", code: "2114" },
    ]
  },
  {
    name: "吉林省",
    code: "22",
    cities: [
      { name: "长春市", code: "2201" },
      { name: "吉林市", code: "2202" },
      { name: "四平市", code: "2203" },
      { name: "辽源市", code: "2204" },
      { name: "通化市", code: "2205" },
      { name: "白山市", code: "2206" },
      { name: "松原市", code: "2207" },
      { name: "白城市", code: "2208" },
      { name: "延边州", code: "2224" },
    ]
  },
  {
    name: "黑龙江省",
    code: "23",
    cities: [
      { name: "哈尔滨市", code: "2301" },
      { name: "齐齐哈尔市", code: "2302" },
      { name: "鸡西市", code: "2303" },
      { name: "鹤岗市", code: "2304" },
      { name: "双鸭山市", code: "2305" },
      { name: "大庆市", code: "2306" },
      { name: "伊春市", code: "2307" },
      { name: "佳木斯市", code: "2308" },
      { name: "七台河市", code: "2309" },
      { name: "牡丹江市", code: "2310" },
      { name: "黑河市", code: "2311" },
      { name: "绥化市", code: "2312" },
    ]
  },
  {
    name: "陕西省",
    code: "61",
    cities: [
      { name: "西安市", code: "6101" },
      { name: "铜川市", code: "6102" },
      { name: "宝鸡市", code: "6103" },
      { name: "咸阳市", code: "6104" },
      { name: "渭南市", code: "6105" },
      { name: "延安市", code: "6106" },
      { name: "汉中市", code: "6107" },
      { name: "榆林市", code: "6108" },
      { name: "安康市", code: "6109" },
      { name: "商洛市", code: "6110" },
    ]
  },
  {
    name: "甘肃省",
    code: "62",
    cities: [
      { name: "兰州市", code: "6201" },
      { name: "嘉峪关市", code: "6202" },
      { name: "金昌市", code: "6203" },
      { name: "白银市", code: "6204" },
      { name: "天水市", code: "6205" },
      { name: "武威市", code: "6206" },
      { name: "张掖市", code: "6207" },
      { name: "平凉市", code: "6208" },
      { name: "酒泉市", code: "6209" },
      { name: "庆阳市", code: "6210" },
      { name: "定西市", code: "6211" },
      { name: "陇南市", code: "6212" },
    ]
  },
  {
    name: "青海省",
    code: "63",
    cities: [
      { name: "西宁市", code: "6301" },
      { name: "海东市", code: "6302" },
      { name: "海北州", code: "6322" },
      { name: "黄南州", code: "6323" },
      { name: "海南州", code: "6325" },
      { name: "果洛州", code: "6326" },
      { name: "玉树州", code: "6327" },
      { name: "海西州", code: "6328" },
    ]
  },
  {
    name: "云南省",
    code: "53",
    cities: [
      { name: "昆明市", code: "5301" },
      { name: "曲靖市", code: "5303" },
      { name: "玉溪市", code: "5304" },
      { name: "保山市", code: "5305" },
      { name: "昭通市", code: "5306" },
      { name: "丽江市", code: "5307" },
      { name: "普洱市", code: "5308" },
      { name: "临沧市", code: "5309" },
      { name: "大理州", code: "5329" },
      { name: "红河州", code: "5325" },
      { name: "文山州", code: "5326" },
      { name: "西双版纳州", code: "5328" },
      { name: "楚雄州", code: "5323" },
      { name: "德宏州", code: "5331" },
      { name: "怒江州", code: "5333" },
      { name: "迪庆州", code: "5334" },
    ]
  },
  {
    name: "贵州省",
    code: "52",
    cities: [
      { name: "贵阳市", code: "5201" },
      { name: "六盘水市", code: "5202" },
      { name: "遵义市", code: "5203" },
      { name: "安顺市", code: "5204" },
      { name: "毕节市", code: "5205" },
      { name: "铜仁市", code: "5206" },
      { name: "黔西南州", code: "5223" },
      { name: "黔东南州", code: "5226" },
      { name: "黔南州", code: "5227" },
    ]
  },
  {
    name: "广西壮族自治区",
    code: "45",
    cities: [
      { name: "南宁市", code: "4501" },
      { name: "柳州市", code: "4502" },
      { name: "桂林市", code: "4503" },
      { name: "梧州市", code: "4504" },
      { name: "北海市", code: "4505" },
      { name: "防城港市", code: "4506" },
      { name: "钦州市", code: "4507" },
      { name: "贵港市", code: "4508" },
      { name: "玉林市", code: "4509" },
      { name: "百色市", code: "4510" },
      { name: "贺州市", code: "4511" },
      { name: "河池市", code: "4512" },
      { name: "来宾市", code: "4513" },
      { name: "崇左市", code: "4514" },
    ]
  },
  {
    name: "海南省",
    code: "46",
    cities: [
      { name: "海口市", code: "4601" },
      { name: "三亚市", code: "4602" },
      { name: "三沙市", code: "4603" },
      { name: "儋州市", code: "4604" },
    ]
  },
  {
    name: "内蒙古自治区",
    code: "15",
    cities: [
      { name: "呼和浩特市", code: "1501" },
      { name: "包头市", code: "1502" },
      { name: "乌海市", code: "1503" },
      { name: "赤峰市", code: "1504" },
      { name: "通辽市", code: "1505" },
      { name: "鄂尔多斯市", code: "1506" },
      { name: "呼伦贝尔市", code: "1507" },
      { name: "巴彦淖尔市", code: "1508" },
      { name: "乌兰察布市", code: "1509" },
    ]
  },
  {
    name: "西藏自治区",
    code: "54",
    cities: [
      { name: "拉萨市", code: "5401" },
      { name: "日喀则市", code: "5402" },
      { name: "昌都市", code: "5403" },
      { name: "林芝市", code: "5404" },
      { name: "山南市", code: "5405" },
      { name: "那曲市", code: "5406" },
    ]
  },
  {
    name: "宁夏回族自治区",
    code: "64",
    cities: [
      { name: "银川市", code: "6401" },
      { name: "石嘴山市", code: "6402" },
      { name: "吴忠市", code: "6403" },
      { name: "固原市", code: "6404" },
      { name: "中卫市", code: "6405" },
    ]
  },
  {
    name: "新疆维吾尔自治区",
    code: "65",
    cities: [
      { name: "乌鲁木齐市", code: "6501" },
      { name: "克拉玛依市", code: "6502" },
      { name: "吐鲁番市", code: "6504" },
      { name: "哈密市", code: "6505" },
      { name: "昌吉州", code: "6523" },
      { name: "博尔塔拉州", code: "6527" },
      { name: "巴音郭楞州", code: "6528" },
      { name: "阿克苏地区", code: "6529" },
      { name: "克孜勒苏州", code: "6530" },
      { name: "喀什地区", code: "6531" },
      { name: "和田地区", code: "6532" },
      { name: "伊犁州", code: "6540" },
      { name: "塔城地区", code: "6542" },
      { name: "阿勒泰地区", code: "6543" },
    ]
  },
  
  // 特别行政区
  {
    name: "香港特别行政区",
    code: "81",
    cities: [
      { name: "香港", code: "8100" }
    ]
  },
  {
    name: "澳门特别行政区",
    code: "82",
    cities: [
      { name: "澳门", code: "8200" }
    ]
  },
  {
    name: "台湾省",
    code: "71",
    cities: [
      { name: "台北市", code: "7101" },
      { name: "新北市", code: "7102" },
      { name: "桃园市", code: "7103" },
      { name: "台中市", code: "7104" },
      { name: "台南市", code: "7105" },
      { name: "高雄市", code: "7106" },
      { name: "基隆市", code: "7107" },
      { name: "新竹市", code: "7108" },
      { name: "嘉义市", code: "7109" },
    ]
  },
  
  // 海外选项
  {
    name: "海外",
    code: "99",
    cities: [
      { name: "北美", code: "9901" },
      { name: "欧洲", code: "9902" },
      { name: "亚洲其他", code: "9903" },
      { name: "大洋洲", code: "9904" },
      { name: "其他地区", code: "9999" },
    ]
  },
];

// 获取省份列表
export function getProvinces(): Province[] {
  return chinaRegions;
}

// 根据省份获取城市列表
export function getCitiesByProvince(provinceName: string): City[] {
  const province = chinaRegions.find(p => p.name === provinceName);
  return province?.cities || [];
}

// 格式化家乡显示
export function formatHometown(province: string, city: string): string {
  if (!province) return '';
  if (!city || province === city) return province.replace(/省|市|自治区|特别行政区/g, '');
  return `${province.replace(/省|自治区|特别行政区/g, '')} ${city.replace(/市|州|地区/g, '')}`;
}
